...
def accumulated_bleu(reference, decodes, sorted_key, uncertainty, name, n=0):
  tf.logging.info("Calculating the accumulated BLEU scores ordered by " + name) 
  sorted_ref = [reference[sorted_key[index]] for index in range(len(sorted_key))]
  sorted_hyp = [decodes[sorted_key[index]] for index in range(len(sorted_key))]
  ...
  if n > 0:
    save_top_bottom_n_lines(sorted_ref, sorted_hyp, uncertainty, individual_bleus, n, name)
...
def main(_):
  ...
  accumulated_bleu(reference, decodes, sorted_bs_scores_key, bs_scores, "bs_scores", 20)
  ...
  accumulated_bleu(reference, decodes, sorted_mc_scores_key, mc_scores, "mc_scores", 20)
