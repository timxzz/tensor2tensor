...
def compute_topk_scores_and_seq(sequences,
  ...
  if not use_tpu:
    ...
    if log_probs_to_gather is not None:
      topk_gathered_log_probs = gather(log_probs_to_gather, "_topk_log_probs")
    ...
    topk_gathered_scores = fast_tpu_gather(scores_to_gather, topk_indexes,
                                           prefix + "_topk_scores")
    if log_probs_to_gather is not None:
      topk_gathered_log_probs = fast_tpu_gather(log_probs_to_gather, topk_indexes,
                                           prefix + "_topk_log_probs")
  ...
  if topk_gathered_log_probs is None:
    return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_states
  else:
    return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_log_probs, topk_gathered_states
...
def beam_search(symbols_to_logits_fn,
  ...
  finished_log_probs = tf.tile(initial_log_probs, [batch_size, 1])
  ...
  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
                    curr_scores, curr_finished, finished_log_probs, curr_log_probs):
    """Given sequences and scores, will gather the top k=beam size sequences.
    ...
    curr_log_probs += (1. - tf.to_float(curr_finished)) * -INF
    ...
    curr_log_probs = tf.concat([finished_log_probs, curr_log_probs], axis=1)
    return compute_topk_scores_and_seq(
        ...
        log_probs_to_gather=curr_log_probs)
  ...
  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, finished_log_probs, states):
    ...
    finished_seq, finished_scores, finished_flags, finished_log_probs, _ = grow_finished(
        finished_seq, finished_scores, finished_flags, topk_seq, topk_scores,
        topk_finished, finished_log_probs, topk_log_probs)

    return (i + 1, alive_seq, alive_log_probs, finished_seq, finished_scores,
            finished_flags, finished_log_probs, states)

  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   finished_scores, unused_finished_in_finished, unused_finished_log_probs,
                   unused_states):
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, finished_log_probs, states) = tf.while_loop(
       _is_finished,
       inner_loop, [
           tf.constant(0), alive_seq, alive_log_probs, finished_seq,
           finished_scores, finished_flags, finished_log_probs, states
       ],
       shape_invariants=[
           tf.TensorShape([]),
           inner_shape,
  ...
  return finished_seq, finished_scores, states, finished_log_probs
